...
class MemberController extends Controller
{
    ...
    public function register()
    {
        ...
        if (IS_POST) {
            ...
            {
                $this->error("密码必须包含字母和数字");
            }
            if(preg_match("/^[a-z]*$/i",$aPassword))
            {
                $this->error("密码必须包含字母和数字");
            }
            //注册用户
            ...
            if (0 < $uid) { //注册成功
                ...
                D('Member')->login($uid, false, $aRole); //登录
...
//            邀请码渲染
                    ...
                    if ($invite['end_time'] > time()) {
                        ...
                        if ($invite_type) {
                            ...
                            if ($roleUser) {
                                ...
                                $memberModel->login($uid, false, $aRoleId); //登录
                                $result['status'] = 1;
     ...
     * @author:xjw129xjt(肖骏涛) xjt@ourstu.com
     */
    public function doSendVerify($account, $verify, $type,$type_other)
    {
        switch ($type) {
            case 'mobile':
                if($type_other==1){
                    ...
                    $res = sendSMS($account, $content,$type_other);
                    return $res;
                }elseif($type_other==2){
                    $content = modC('SMS_CONTENT_PSW', '{$verify}', 'USERCONFIG');
                    $content = str_replace('{$verify}', $verify, $content);
                    $content = str_replace('{$account}', $account, $content);
                    $res = sendSMS($account, $content,$type_other);
                    return $res;
                }elseif($type_other==3){
                    $content = modC('SMS_CONTENT_MOB', '{$verify}', 'USERCONFIG');
                    $content = str_replace('{$verify}', $verify, $content);
                    $content = str_replace('{$account}', $account, $content);
                    $res = sendSMS($account, $content,$type_other);
                    return $res;
...
//                    $this->error(L('_ERROR_EMAIL_LENGTH_LIMIT_'));
    ...
    public function checkNickname()
    {
        ...
        preg_match('/^[\x{4e00}-\x{9fa5}_a-zA-Z0-9]+$/u', $aNickname, $result);
        if (!$result) {
