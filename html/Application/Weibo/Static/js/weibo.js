    ...
    $('[data-role="do_comment"]').click(function () {
        ...
        $.post(url, {weibo_id: weiboId, content: content, comment_id: comment_id,position:position}, function (a) {
            ...
            if (a.status) {
                ...
                $('.XT_face').remove();
                weibo_bind();
                var num = parseInt($('.comment-count').text());
                $('.comment-count').text(num + 1);
            } else {
...
var weibo_set_top = function(){
    ...
    $('[data-role="weibo_set_top"]').click(function () {
        ...
        var top_title = $('input[name="top_title"]').val() ;
        var top_day = $('input[name="top_dead"]').val() ;
        var weiboId = $(this).attr('data-weibo-id');
        $.post(U('weibo/index/setTop'), {weibo_id: weiboId,title:top_title,day:top_day,type:top_type}, function (msg) {
            if (msg.status) {
...
function bind_comment_support() {
    ...
    $('[data-role="support_btn"]').click(function () {
        ...
        var me = $(this);
        if (MID == 0) {
            toast.error('请在登录后再点赞。', L('_KINDLY_REMINDER_'));
            return;
        } else {
...
var clear_weibo = function (value) {
    $('.weibo_post_box #weibo_content').val(value);
}
...
var do_send_repost = function(){
    ...
    $('[data-role="do_send_repost"]').click(function () {
        ...
        $.post(url, {content: content,type:feedType,sourceId:sourceId,weiboId:weiboId,becomment:becomment[0].checked}, function (a) {
            ...
            if (a.status) {
                //同步数据
                var repost_count = parseInt($('.repost-count').text());
                $('.repost-count').text(repost_count + 1);
                $('.mfp-close').click();
            ...
            success: function (a) {
                switch (a) {
                    ...
                    default :
                        ...
                        var html;
                        for (var k in a) {
                            html = '<a href="javascript:void(0)" data-type="' + a[k]['name'] + '" title="' + a[k]['from'] + '" onmousedown="face_chose(event,$(this))";><img src="' + a[k]['path'] + '" width="60" height="60" /></a>  ';
                            $('.imghtml').after(html);
                        }
                        toast.success('添加成功!');
                }
