...
class AdminController extends Controller
{
     ...
     * @author 朱亚杰  <zhuyajie@topthink.net>
     */
    protected function resume($model, $where = array(), $msg = array('success' => '状态恢复成功！', 'error' => '状态恢复失败！'), $data=array('status' => 1))
    {
        $this->editRow($model, $data, $where, $msg);
    }
    ...
    public function getModules()
    {
        ...
        if ($modules === false) {
            ...
            foreach ($modules as &$v) {
                if ($v['is_setup']) {
                    $i++;
                    $v['word'] = mb_substr($v['alias'], 0, 1, 'utf8');
                ...
                foreach ($menus['main'] as $key => $item) {
                    ...
                    if ($item['title'] == $nav_first_title) {
                        ...
                        foreach ($groups as $g) {
                            ...
                            $menuList = M('Menu')->where($map)->field('id,pid,title,url,tip,module')->order('sort asc')->select();
                            $menus['child'][$g] = list_to_tree($menuList, 'id', 'pid', 'operater', $item['id']);
                            if ($menus['child']['module'] == false) {//判断一级导航展开@author szh@ourstu.com
                                $menus['child']['module'] = $menuList[0]['module'];
                            }
                        }
