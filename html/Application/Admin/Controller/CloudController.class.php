...
class CloudController extends AdminController
{
    ...
    public function getFileList()
    {
        ...
        S('nextVersion', json_encode($nextVersion));
        S('currentVersion', json_encode($currentVersion));
    }
    ...
    public function compare()
    {
        ...
        $currentVersion = json_decode(S('currentVersion'), true);
        $nextVersion = json_decode(S('nextVersion'), true);
        $old_file_path = C('UPDATE_PATH') . $nextVersion['name'] . '/old';
        $new_file_path = C('UPDATE_PATH') . $nextVersion['name'] . '/new';
    ...
    public function updateGoods()
    {
        ...
        $this->assign('token', $aToken);
        $this->assign('version', $version);
        S('version',json_encode($version));
        S('versionList',json_encode($versionList));
        S('token',$aToken);
    ...
    {
        /*初始化各类信息*/
        $version = json_decode(S('version'),1);
        $versionList = array_reverse(json_decode(S('versionList'),1));
        $token = S('token');
        $this->assign('version', $version);
        $this->assign('versionList', $versionList);
        if (empty($version)) {
