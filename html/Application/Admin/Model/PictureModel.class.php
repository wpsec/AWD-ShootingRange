...
class PictureModel extends Model{
    ...
    public function upload($files, $setting, $driver = 'local', $config = null){
        ...
        //        开始解决手机上传的图片发布出去可能存在翻转问题
        if(extension_loaded(exif) == true){
            if($driver != 'local'){
                $pathImg = $info['file']['url'];
                $file_arr = exif_read_data($pathImg);
                if($file_arr['Orientation']){
                    switch($file_arr['Orientation']){
                        case 8:
                            $info['file']['url'] = $info['file']['url'].'?imageMogr2/rotate/-90';
                            break;
                        case 3:
                            $info['file']['url'] = $info['file']['url'].'?imageMogr2/rotate/180';
                            break;
                        case 6:
                            $info['file']['url'] = $info['file']['url'].'?imageMogr2/rotate/90';
                            break;
                ...
                $pathImg = $info['file']['rootPath'] . $info['file']['savepath'] . $info['file']['savename'];
                $file_arr = exif_read_data($pathImg);
                ...
                if (!empty($file_arr)) {
                    ...
                    switch ($info['file']['type']){
                        case "image/jpeg":
                            imagejpeg($image, $pathImg);
                            break;
                        case "image/png":
                            imagepng($image, $pathImg);
                            break;
                        case "image/gif":
                            imagegif($image, $pathImg);
                            break;
                        default: break;
                    }
                }
    ...
    public function isFile($file){
        ...
        /* 查找文件 */
		$map = array('md5' => $file['md5'],'sha1'=>$file['sha1'],'type'=>$this->fileDriver);
