...
class IndexController extends Controller
{
    ...
    public function addComment()
    {
      ...
      // $uid=is_login();
       ...
       /* $map['app']=$aApp;
        ...
        $data = array('app' => $aApp, 'mod' => $aMod, 'row_id' => $aRowId, 'content' => $aContent, 'uid' => is_login(), 'ip' => get_client_ip(1), 'area' => '');
        ...
        if ($res) {
            ...
            $object = new $class;
            $html = parse_at_users($object->getCommentHtml($res));
            if (!is_login()) {
                if ($aUid) {
                    $title = '游客' . '评论了您';
    ...
    public function getCommentList()
    {
        ...
        $object = new $class;
        foreach ($list as $v) {
            $html .= parse_at_users($object->getCommentHtml($v));
        }
