...
class ueditor_control extends phpok_control
{
	...
	private function load_config()
	{
		...
		$tmp = array('localhost','127.0.0.1','img.baidu.com',$domain);
	...
	**/
	...
	private function u_listvideo()
	{
		...
		foreach($rslist as $key=>$value){
			$tmp = array('url'=>$value['filename'],'ico'=>$value['ico'],'mtime'=>$value['addtime'],'title'=>$value['title'],'name'=>$value['title'],'original'=>$value['title']);
			$piclist[] = $tmp;
		}
	...
	private function u_listfile()
	{
		...
		$piclist = array();
		foreach($rslist as $key=>$value){
			$tmp = array('url'=>$value['filename'],'ico'=>$value['ico'],'mtime'=>$value['addtime'],'title'=>$value['title'],'id'=>$value['id'],'name'=>$value['title'],'original'=>$value['title']);
			$piclist[] = $tmp;
		}
		$data = array('list'=>$piclist,'start'=>$offset,'size'=>$psize);
		$this->_stop(true,$data);
	...
	private function u_uploadfile()
	{
		...
		if(!$rs || $rs['status'] != 'ok'){
			$tip = $rs['error'] ? $rs['error'] : P_Lang('附件类型不符合要求');
			$this->_stop($tip);
		}
		$data = array('title'=>$rs['title'],'url'=>$rs['filename'],'original'=>$rs['title'],'id'=>$rs['id'],'name'=>$value['title']);
		$this->_stop(true,$data);
	}
	...
	private function u_uploadimage()
	{
		...
		$data = array('title'=>$rs['title'],'url'=>$filename,'original'=>$filename,'id'=>$rs['id'],'name'=>$rs['title']);
		$this->_stop(true,$data);
	}
